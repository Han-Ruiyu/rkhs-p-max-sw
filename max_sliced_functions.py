...
def expected_max_swdistance_empirical_covar_Gaussian(d,N,p, n_montecarlo=100):
    '''
    Compute the expection of p-max-sliced Wasserstein distance
    ...
    cov = 0.5*np.diag(np.ones(d))+0.5*np.ones([d,d])
...
def pushforward_embed_into_gaussian_rkhs(sample, theta, d_test, L, sigma_square=1, omega_square=2):
    ...
    eigenvalue_base=8/(9+np.sqrt(17))
    
    #need to compute the eigenvalue
    ...
    eigenvalue=np.sqrt(eigenvalue_base/4)*np.power(eigenvalue_base,index)
    
    const_coeff=const_coeff*eigenvalue
...
def max_swdistance_embed_into_gaussian_rkhs(I0,I1,N, p, d_test, L=1000):
    '''
...
def expected_max_swdistance_empirical_embed_into_gaussian_rkhs(distribution,p,N, sigma,d_test,n_montecarlo):
    '''
