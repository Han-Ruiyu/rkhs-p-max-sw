...
for i in range(5):
    ...
    #max_sw_distance_d10[i]=expected_max_swdistance_empirical_embed_into_gaussian_rkhs('Gaussian',2,num,30)
    
    max_sw_distance_12[i]=expected_max_swdistance_empirical_embed_into_gaussian_rkhs('Gaussian',2,num,1/2)
    max_sw_distance_11[i]=expected_max_swdistance_empirical_embed_into_gaussian_rkhs('Gaussian',2,num,1)
...
plt.plot(number_of_points,max_sw_distance_11)
plt.plot(number_of_points, max_sw_distance_12)
plt.plot(number_of_points, max_sw_distance_1squareroot2)
plt.plot(number_of_points, np.power(np.log(number_of_points),11/12)*np.power(number_of_points,-1/4))
plt.title('${\mu}=v=\mathcal{N}(0,\eta^2)$')
plt.xlabel('number of sample points $n$',fontsize=18)

plt.ylabel('$\mathbb{E}\overline{W}_2(\Phi_{\#}(\mu_n),\Phi_{\#}(\mu))$',fontsize=16)
plt.legend(['$\eta^2=1$','$\eta^2=1/4$','$\eta^2=1/2$', '$C*n^{-1/4}(\log n)^{11/12}$'],loc='center right',bbox_to_anchor=(1.45, 0.8))
plt.grid()
